...
export default function AdminUsersPage() {
  ...
  const save = async () => {
    ...
    toast.success(editId ? "User berhasil diperbarui" : "User berhasil ditambahkan");
    setModal(null);
    fetchUsers();
  };
  ...
  return (
    <div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
                <th className="px-4 py-3 text-left font-semibold text-slate-600 dark:text-slate-400">User</th>
                <th className="px-4 py-3 text-left font-semibold text-slate-600 dark:text-slate-400">Email</th>
                <th className="px-4 py-3 text-left font-semibold text-slate-600 dark:text-slate-400">Role</th>
                <th className="px-4 py-3 text-right font-semibold text-slate-600 dark:text-slate-400">Aksi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {filtered.length === 0 ? (
                <tr><td colSpan={4} className="px-4 py-12 text-center text-slate-500">Tidak ada user</td></tr>
              ) : (
                filtered.map((u) => (
                  <tr key={u.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-1">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => handleDelete(u.id)} className="text-red-600 hover:text-red-700">Hapus</Button>
                      </div>
                    </td>
                  </tr>
                ))
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={modal === "edit" ? "Edit User" : "Tambah User"}>
        <div className="space-y-4">
          ...
          />
          <Input
            label={modal === "edit" ? "Password Baru (kosongkan jika tidak diubah)" : "Password"}
            type="password"
            value={form.password}
